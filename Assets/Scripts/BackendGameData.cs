...
public class UserData
{
    ...
    // Custom : 강좌를 위해 추가 (다른 유저 데이터는 사용하지 않지만, 삭제하지 않겠습니다)
    public int[] scores;
    ...
    public override string ToString()
    {
        ...
        // Custom : 강좌를 위해 추가 (다른 유저 데이터는 사용하지 않지만, 삭제하지 않겠습니다)
        result.AppendLine($"scores");
        if (scores != null)
        {
            for (int i = 0; i < scores.Length; i++)
            {
                result.AppendLine($"| Score{i + 1} : {scores[i]}");
            }
        }
...
public class BackendGameData
{
    ...
    public void GameDataInsert()
    {
        ...
        userData.inventory.Add("하야포션", 1);
        userData.inventory.Add("파란포션", 1);

        // 추가
        userData.scores = new int[] { 0, 0, 0 };
        ...
        // 추가
        param.Add("scores", userData.scores);
    ...
    public void GameDataGet()
    {
        ...
        if (bro.IsSuccess())
        {
            ...
            {
                Debug.LogWarning("데이터가 존재하지 않습니다. 기본 데이터를 생성합니다.");
            ...
            else
            {
                ...
                if (gameDataJson[0].ContainsKey("scores"))
                {
                    LitJson.JsonData jsonScores = gameDataJson[0]["scores"];

                    userData.scores = new int[jsonScores.Count];

                    for (int i = 0; i < jsonScores.Count; i++)
                    {
                        userData.scores[i] = int.Parse(jsonScores[i].ToString());
                    }
                }
